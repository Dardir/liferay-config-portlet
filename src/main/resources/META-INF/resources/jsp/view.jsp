				...
				<div class="wrap-table100">
					<div class="table100">
						<aui:button value="Add" onClick="<%=addConfig.toString()%>"
							name="addButton" cssClass="btn-info btn-save btn-add"
							...
							<tbody>
								<c:forEach var="configItem" items="${configurations}">
									<portlet:renderURL var="editEntryURL">
										<portlet:param name="idConfig" value="${configItem.key}" />
										<portlet:param name="mvcPath" value="/jsp/editConfig.jsp" />
									</portlet:renderURL>
									<portlet:renderURL var="deleteEntryURL">
										<portlet:param name="idConfig" value="${configItem.key}" />
										<portlet:param name="mvcPath" value="/jsp/deleteConfig.jsp" />
									</portlet:renderURL>
									<tr class="row100 body">
										<td class="column3">${configItem.key}</td>
										<td class="column3">${configItem.value}</td>
										<td class="column7"><aui:button value="Edit"
												onClick="<%=editEntryURL.toString()%>" name="editButton"
												icon="icon-edit" iconAlign="right" />
										<aui:button value="Delete"
												onClick="<%=deleteEntryURL.toString()%>" name="deleteButton"
												icon="icon-trash" iconAlign="right" /></td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
						<aui:button type="submit" value="Save"
							cssClass="btn-info btn-save" icon="icon-save" iconAlign="right" />
					</div>
				</div>
			</div>
		</div>
